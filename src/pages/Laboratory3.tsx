...
import { Laboratory3Agents } from "./Laboratory3Agents";
import { Laboratory3Data } from "./Laboratory3Data";
import { Workflow, Database } from "lucide-react";

export default function Laboratory3() {
  const { t } = useLanguage();
  const { isDeveloperMode } = useDeveloperMode();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<"agents" | "data">("agents");

  // Require developer mode to access Laboratory3.0
  useEffect(() => {
    if (!isDeveloperMode) {
      navigate('/dashboard');
    }
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader 
        title="Laboratory3.0" 
        subtitle="Advanced AI/ML Workflow Platform"
      />
