...
export const HeroSection = ({
  ...
  return <section className="relative min-h-screen flex items-center justify-center bg-gradient-hero">
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button variant="hero" size="lg" onClick={onLoginClick} className="text-lg px-8 py-4 text-slate-950">
              AI-HUB <ArrowRight className="ml-2 h-5 w-5" />
            </Button>
            <Button variant="outline-hero" size="lg" className="text-lg px-8 py-4 text-slate-50">
              Узнать больше
            </Button>
          </div>
        </div>
      ...
      <div className="absolute top-1/3 right-1/3 w-1 h-1 bg-primary-glow rounded-full animate-float opacity-60" style={{
      animationDelay: '2s'
    }}></div>
      <div className="absolute bottom-1/4 right-1/4 w-3 h-3 bg-primary rounded-full animate-float opacity-50" style={{
      animationDelay: '4s'
    }}></div>
    </section>;
};