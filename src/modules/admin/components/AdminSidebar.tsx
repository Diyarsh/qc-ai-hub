...
export function AdminSidebar({ role }: AdminSidebarProps) {
  return (
    <aside className="w-64 h-screen bg-card text-foreground flex flex-col border-r border-border shadow">
      <div className="flex items-center h-16 px-6 font-bold text-lg tracking-tight border-b border-border">Админ-панель</div>
      <nav className="flex-1 px-2 py-4">
        {adminMenu.filter(item => item.roles.includes(role)).map(({ id, label, icon: Icon, path }) => (
          <NavLink to={path} key={id}
            className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-md my-1 transition-all duration-150 ${isActive ? "bg-muted text-blue-600" : "hover:bg-muted/50 hover:text-blue-600 text-muted-foreground"}`}
          >
